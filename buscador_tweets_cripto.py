...
class TwitterCriptoScraper:
    ...
    def buscar_tweets_por_termos(self, termos: List[str], max_por_termo: int = 3, 
        ...
        """
        Busca tweets para uma lista de termos.
        ...
        for termo in termos:
            ...
            tweets = self.buscar_tweets(termo, max_por_termo)
        ...
        # Ordenar por engajamento (likes + retweets)
        tweets_unicos.sort(key=lambda x: x.get("likes", 0) + x.get("retweets", 0), reverse=True)
    ...
    def buscar_tweets_por_contas(self, contas: List[str], max_por_conta: int = 3,
                               max_total: int = 15) -> List[Dict[str, Any]]:
        """
        Busca tweets de contas específicas.
        ...
        for conta in contas:
            ...
            query = f"from:{conta}"
            tweets = self.buscar_tweets(query, max_por_conta)
            todos_tweets.extend(tweets)
        ...
        todos_tweets.sort(key=lambda x: x.get("created_at", ""), reverse=True)
