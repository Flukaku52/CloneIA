    ...
    def _agrupar_noticias_similares(self, noticias: List[Dict[str, Any]]) -> List[List[Dict[str, Any]]]:
        ...
        for i, noticia1 in enumerate(noticias):
            ...
            for j, noticia2 in enumerate(noticias):
                ...
                similaridade_titulo = self._calcular_similaridade(
                ...
                if noticia1.get('resumo') and noticia2.get('resumo'):
                    similaridade_resumo = self._calcular_similaridade(
                        noticia1['resumo'], noticia2['resumo']
                    )
                
                # Usar a maior similaridade entre título e resumo
                similaridade = max(similaridade_titulo, similaridade_resumo)
                
                # Se a similaridade for maior que o limiar, adicionar ao grupo
