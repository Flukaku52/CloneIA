...
def main():
    ...
    # Gerar o vídeo Rapidinha
    resultado = gerar_rapidinha_completa(
        dias_max=args.dias,
        num_noticias=args.noticias,
        num_tweets=args.tweets,
        dry_run=args.dry_run,
        abrir_resultado=not args.no_abrir
    )
    
    # Exibir resultado
    if resultado["status"] == "success":
        logger.info("Vídeo Rapidinha gerado com sucesso!")
        logger.info(f"Script: {resultado['script_path']}")
        logger.info(f"Áudio: {resultado['audio_path']}")
        logger.info(f"Vídeo: {resultado['video_path']}")
        
        if resultado["dry_run"]:
            logger.info("NOTA: Este foi um teste em modo de simulação. Nenhum recurso de API foi consumido.")
    else:
        logger.error(f"Falha ao gerar vídeo Rapidinha: {resultado['message']}")
