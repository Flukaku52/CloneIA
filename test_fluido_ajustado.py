#!/usr/bin/env python3
"""
Script para testar o perfil de voz 'fluido' ajustado com similarity_boost aumentado para 0.7.
Gera um vídeo com o HeyGen usando o texto otimizado e verifica se a voz ficou mais próxima da original.
"""
import os
import sys
import json
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('test_fluido_ajustado.log')
    ]
)
logger = logging.getLogger('test_fluido_ajustado')

# Importar os módulos necessários
from heygen_video_generator import HeyGenVideoGenerator, open_video_file
from core.text import TextProcessor

# Texto do script fornecido (mesmo do teste anterior)
...
def generate_video(text, avatar_id, folder_name):
    ...
    video_path = os.path.join(output_dir, f"fluido_ajustado_{timestamp}.mp4")
...
def main():
    ...
    """
    logger.info("Iniciando teste do perfil 'fluido' ajustado...")
    ...
    if video_path:
        ...
        # Gerar relatório
        report = [
            "=== RELATÓRIO DE TESTE DO PERFIL 'FLUIDO' AJUSTADO ===",
            f"Data: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
            f"\nTexto utilizado: {SCRIPT_TEXT}",
            f"\nTexto otimizado: {optimized_text}",
            f"\nAvatar utilizado: {AVATAR_ID}",
            f"\nVídeo gerado: {video_path}",
            "\n=== CONFIGURAÇÃO DO PERFIL 'FLUIDO' AJUSTADO ===",
            "- stability: 0.2",
            "- similarity_boost: 0.7 (aumentado de 0.6)",
            "- style: 1.0",
            "- use_speaker_boost: true",
            "- model_id: eleven_multilingual_v2",
            "\n=== OBSERVAÇÕES ===",
            "- O perfil 'fluido' foi ajustado para aumentar a similaridade com a voz original.",
            "- O parâmetro similarity_boost foi aumentado de 0.6 para 0.7.",
            "- Este ajuste deve tornar a voz mais próxima da original, mantendo a fluidez.",
            "- O vídeo foi gerado diretamente com o HeyGen usando o texto otimizado.",
            "- O vídeo foi reproduzido automaticamente para análise."
        ]
        
        # Salvar o relatório
        report_path = os.path.join(os.getcwd(), "output", f"relatorio_fluido_ajustado_{int(time.time())}.txt")
